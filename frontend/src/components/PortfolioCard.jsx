...
export default function PortfolioCard({ holding }) {
  ...
  return (
    <div className="group relative overflow-hidden bg-darkBorder/50 border border-darkBorder hover:border-blue-500/50 rounded-lg p-4 transition-all duration-200 hover:shadow-lg hover:shadow-blue-500/10">
      ...
      <div className="relative flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              ...
              {dayChange !== 0 && (
                <span className={`text-xs px-1.5 py-0.5 rounded animate-pulse ${
                  isDayPositive ? 'bg-green-500/20 text-green-400' : 'bg-red-500/20 text-red-400'
                }`}>
                  {isDayPositive ? '+' : ''}{dayChange.toFixed(2)}%
                </span>
              )}
            </div>
            <p className="text-xs text-gray-400 truncate">{holding.company}</p>
            <p className="text-xs text-gray-500 mt-1">
              {quantity} shares @ <span className="font-semibold text-white">${currentPrice.toFixed(2)}</span>
            </p>
          </div>
        </div>
        ...
        <div className="text-right">
          <p className="text-lg font-bold text-white transition-all group-hover:scale-105">
      ...
      <div className="absolute bottom-0 left-0 right-0 h-0.5 bg-gray-700/30">
        <div 
          className={`h-full transition-all duration-500 ${
            isGainPositive ? 'bg-gradient-to-r from-green-500 to-emerald-400' : 'bg-gradient-to-r from-red-500 to-rose-400'
          }`}
