...
export default function AlertCard({ alert, onExplain }) {
  ...
  const chain = alert.chain || `${alert.impactChain?.level1 || ''} → ${alert.impactChain?.level2 || ''} → ${alert.impactChain?.level3 || ''}`
    ...
    <div className={`group relative overflow-hidden rounded-xl border-l-4 p-6 backdrop-blur-sm transition-all hover:shadow-2xl ${styles.border} ${styles.bg} ${styles.hover}`}>
      ...
      <div className="flex items-start justify-between mb-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-gray-400">{new Date(created_at).toLocaleString()}</p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-2 gap-4 mb-4">
        <div className="rounded-lg bg-black/20 p-3">
          ...
          <p className={`text-2xl font-bold ${impact_percent > 0 ? 'text-green-400' : 'text-red-400'}`}>
      ...
      {affected_holdings.length > 0 && (
        <div className="mb-4">
          <p className="text-sm font-semibold text-white mb-2">💼 Affected Holdings:</p>
      ...
      {sources.length > 0 && (
        <div className="mb-4">
          <p className="text-sm font-semibold text-white mb-2">📰 Sources:</p>
          <div className="space-y-2">
